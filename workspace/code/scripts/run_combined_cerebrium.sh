#!/usr/bin/env bash
#
# Async-invoke the deployed CacheBlend *combined* accuracy+TTFT measurement.
#
# Targets the run_combined_cerebrium function, which loads the model once and
# measures TTFT FIRST (on a clean device) then accuracy -- so the accuracy
# phase's full-length generation cannot perturb the TTFT numbers. Adds the same
# --repeats / --warmup knobs as run_ttft_cerebrium.sh. As with the other
...
#
# Usage:
#   ./run_combined_cerebrium.sh                              # mode=smoke (async)
#   ./run_combined_cerebrium.sh --mode full --n 50
#   ./run_combined_cerebrium.sh --mode smoke --repeats 5 --warmup 2
#   SYNC=1 ./run_combined_cerebrium.sh                       # wait for the JSON response inline
#   DRY_RUN=1 ./run_combined_cerebrium.sh --mode full        # print the request, don't send it
#
# Required env:
...
#   CEREBRIUM_FUNCTION                default: run_combined_cerebrium
#
...
FUNC="${CEREBRIUM_FUNCTION:-run_combined_cerebrium}"
...
if [[ "${SYNC:-}" == "1" ]]; then
  url="$base"
  echo "[run_combined_cerebrium] SYNC mode: waiting for the response (no webhook)" >&2
else
  url="${base}?async=true"
  if [[ -n "${CEREBRIUM_WEBHOOK_URL:-}" ]]; then
    ...
    echo "[run_combined_cerebrium] async + webhook -> ${CEREBRIUM_WEBHOOK_URL}" >&2
  else
    echo "[run_combined_cerebrium] async (no CEREBRIUM_WEBHOOK_URL set -> run stays 'processing' until you pull the result JSON)" >&2
  fi
fi

echo "[run_combined_cerebrium] POST ${url}" >&2
echo "[run_combined_cerebrium] body ${body}" >&2

if [[ "${DRY_RUN:-}" == "1" ]]; then
  echo "[run_combined_cerebrium] DRY_RUN=1 -> not sending" >&2
  exit 0
fi
